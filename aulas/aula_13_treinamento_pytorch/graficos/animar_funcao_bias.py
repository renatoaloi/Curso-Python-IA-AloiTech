...
def draw_frame(frame_idx, w, b, x_list, scale_x, scale_y):
    # 1. Preparar Fontes
    try:
    ...
    # Desenhar Reta y = wx + b
    x_start, x_end = -1, 20
    p_start = (OFFSET_X + x_start * scale_x, OFFSET_Y - (w * x_start + b) * scale_y)
    p_end = (OFFSET_X + x_end * scale_x, OFFSET_Y - (w * x_end + b) * scale_y)
    ...
    draw.text((OFFSET_X - 120, py - 15), f"y={current_y:.2f}", fill=TEXT_COLOR, font=font_info)

    # Painel de Informações (Topo)
    legenda = f"y = ({w} * {current_x}) + {b} = {current_y:.2f}"
    ...
    # Parâmetros
    draw.text((OFFSET_X + 50, 100), f"w = {w}", fill=TEXT_COLOR, font=font_info)
    draw.text((OFFSET_X + 50, 140), f"b = {b}", fill=LINE_COLOR, font=font_info)

    img.save(f"{TEMP_DIR}/frame_{frame_idx:04d}.png")

def main():
    print("--- ANIMADOR DE FUNÇÃO COM BIAS (y = wx + b) ---")
    try:
        w = float(input("Digite o valor do peso (w): "))
        b = float(input("Digite o valor do bias (b): "))
        x_str = input("Digite a lista de valores de X separados por vírgula: ")
        x_list = [float(x.strip()) for x in x_str.split(",")]
    except ValueError:
        print("Valores inválidos! Usando padrões.")
        w, b, x_list = 2.0, 5.0, [1, 2, 3, 4, 5]
    ...
    OUTPUT_VIDEO = os.path.join(videos_dir, f"bias_anim_w{w}_b{b}_{timestamp}.mp4")

    if not os.path.exists(TEMP_DIR):
        os.makedirs(TEMP_DIR)
        
    print(f"Gerando animação para y = {w}x + {b}...")
    
    for i in range(TOTAL_FRAMES):
        draw_frame(i, w, b, x_list, scale_x, scale_y)
        if i % 30 == 0:
